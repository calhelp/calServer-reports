...
| `ReportVariantCode` | ➖ | `""` | Ressourcenname für die Layoutsteuerung. Wenn in `resource.report_template` für diesen Namen der Wert `1` steht, wird Variante `1` (mit Lücken) aktiviert; sonst läuft Variante `0` (Standard). Ist `ReportVariantCode` leer, bleibt der Report in Variante `0` (Standard). |
